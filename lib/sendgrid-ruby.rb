...
module SendGrid
  class Client
    ...
    def send(mail)
      # TODO: This feels off possibly
      @conn[@endpoint].post(mail.to_h.merge({api_user: @api_user, api_key: @api_key}), {user_agent: 'sendgrid/' + SendGrid::VERSION + ';ruby'}) do |response, request, result|
